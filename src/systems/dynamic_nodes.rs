...
// ) {
//     let unit_node = unit_node.get_instance_if_sane();
//
...
//             Ok(node2d) => {
//                 let test = Ref::<Node2D, Unique>::new();
//
...
pub fn instance_scene<Root>(scene: &PackedScene) -> Result<GodotInstance<Root>, ManageErrs>
where
    Root: gdnative::GodotObject<RefKind = ManuallyManaged> + SubClass<Node>,
{
    ...
    instance
        .try_cast::<Root>()
        .map(|instance| GodotInstance::new(instance.into_shared()))
